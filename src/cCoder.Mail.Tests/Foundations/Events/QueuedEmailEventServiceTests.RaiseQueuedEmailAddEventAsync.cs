...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseQueuedEmailAddEventAsync()
    {
        // Given
        ...
        queuedEmailEventBrokerMock
            .Setup(expression: x => x.RaiseQueuedEmailAddEventAsync(message: It.IsAny<EventMessage<cCoder.Data.Models.Mail.QueuedEmail>>()))
...
expression: x => x.RaiseQueuedEmailAddEventAsync(message: It.IsAny<EventMessage<cCoder.Data.Models.Mail.QueuedEmail>>()),
times: Times.Once
        );
