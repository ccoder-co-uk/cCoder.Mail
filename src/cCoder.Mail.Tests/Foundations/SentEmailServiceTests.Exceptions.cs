...
public partial class SentEmailServiceTests
{
    ...
    {
        sentEmailBrokerMock.Setup(x => x.GetAllSentEmails()).Throws(exception);
        Action action = () => sentEmailService.GetSentEmail(1);
    ...
    {
        authorizationBrokerMock.Setup(x => x.GetCurrentUser()).Throws(exception);
        Func<Task> action = async () => await sentEmailService.AddSentEmailAsync(new SentEmail { AppId = 7 });
    ...
    {
        sentEmailBrokerMock.Setup(x => x.DeleteAllSentEmailsByAppIdAsync(7)).ThrowsAsync(exception);
        Func<Task> action = async () => await sentEmailService.DeleteAllByAppIdAsync(7);
        (await action.Should().ThrowAsync<Exception>()).Which.Should().BeOfType(expectedType);
    }
}
