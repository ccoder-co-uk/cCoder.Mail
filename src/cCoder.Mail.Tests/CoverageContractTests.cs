...
public sealed partial class CoverageContractTests
{
    ...
    public async Task ShouldExerciseEveryServiceAndExposureContract()
    {
        ...
        Assembly[] assemblies =
        [
            typeof(MailSenderController).Assembly,
            typeof(IMailClient).Assembly
        ];

        Type[] subjectTypes = assemblies
            ...
            .Where(predicate: type =>
                ...
                (type.Namespace?.StartsWith(
                    ...
                    (type.Namespace.Contains(value: ".Services") ||
                        type.Namespace.Contains(value: ".Exposures"))))
            .ToArray();

        Type[] exceptionTypes = assemblies
            .SelectMany(selector: assembly => assembly.GetTypes())
            .Where(predicate: type =>
                type.IsClass &&
                !type.IsAbstract &&
