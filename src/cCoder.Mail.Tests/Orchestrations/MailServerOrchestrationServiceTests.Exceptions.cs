...
public sealed partial class MailServerOrchestrationServiceTests
{
    ...
    {
        mailServerProcessingServiceMock.Setup(x => x.GetMailServer(1)).Throws(exception);
        Action action = () => orchestrationService.GetMailServer(1);
    ...
    {
        mailServerProcessingServiceMock.Setup(x => x.AddMailServerAsync(It.IsAny<MailServer>())).ThrowsAsync(exception);
        Func<Task> action = async () => await orchestrationService.AddMailServerAsync(new MailServer());
    ...
    {
        mailServerProcessingServiceMock.Setup(x => x.DeleteByAppIdAsync(7)).ThrowsAsync(exception);
