...
public sealed partial class MailServerProcessingServiceTests
{
    ...
    {
        mailServerServiceMock.Setup(x => x.GetMailServer(1)).Throws(exception);
        Action action = () => mailServerProcessingService.GetMailServer(1);
    ...
    {
        mailServerServiceMock.Setup(x => x.AddMailServerAsync(It.IsAny<MailServer>())).ThrowsAsync(exception);
        Func<Task> action = async () => await mailServerProcessingService.AddMailServerAsync(new MailServer());
    ...
    {
        mailServerServiceMock.Setup(x => x.DeleteAllByAppIdAsync(7)).ThrowsAsync(exception);
        Func<Task> action = async () => await mailServerProcessingService.DeleteByAppIdAsync(7);
        (await action.Should().ThrowAsync<Exception>()).Which.Should().BeOfType(expectedType);
    }
}
