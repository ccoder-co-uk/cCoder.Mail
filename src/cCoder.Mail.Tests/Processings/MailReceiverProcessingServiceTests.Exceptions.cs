...
public sealed partial class MailReceiverProcessingServiceTests
{
    ...
    public void GetMailReceiverShouldMapException(Exception exception, Type expectedType)
    {
        serviceMock.Setup(x => x.GetMailReceiver(It.IsAny<Guid>())).Throws(exception);
        Action action = () => service.GetMailReceiver(Guid.NewGuid());
        action.Should().Throw<Exception>().Which.Should().BeOfType(expectedType);
    }

    [Theory]
    [MemberData(nameof(ExceptionMappings))]
    public async Task AddMailReceiverAsyncShouldMapExceptionAsync(Exception exception, Type expectedType)
    {
        serviceMock.Setup(x => x.AddMailReceiverAsync(It.IsAny<MailReceiver>())).ThrowsAsync(exception);
        Func<Task> action = async () => await service.AddMailReceiverAsync(new MailReceiver());
