...
public sealed partial class QueuedEmailProcessingServiceTests
{
    ...
    {
        queuedEmailServiceMock.Setup(x => x.GetQueuedEmail(1)).Throws(exception);
        Action action = () => queuedEmailProcessingService.GetQueuedEmail(1);
    ...
    {
        queuedEmailServiceMock.Setup(x => x.AddQueuedEmailAsync(It.IsAny<QueuedEmail>(), false)).ThrowsAsync(exception);
        Func<Task> action = async () => await queuedEmailProcessingService.AddQueuedEmailAsync(new QueuedEmail());
    ...
    {
        queuedEmailServiceMock.Setup(x => x.DeleteAllByAppIdAsync(7)).ThrowsAsync(exception);
        Func<Task> action = async () => await queuedEmailProcessingService.DeleteByAppIdAsync(7);
        (await action.Should().ThrowAsync<Exception>()).Which.Should().BeOfType(expectedType);
    }
}
